...
const { searchdocs, goto, moduleinfo, regenerateCache } =
      client.import({rpc: ['searchdocs', 'goto', 'moduleinfo'], msg: ['regenerateCache']})
...
export function processItem (item) {
  ...
  item.onClickName = () => {
    ...
    goto({
      word: item.name,
      mod: item.mod,
      path: editor.getGrammar() === grammar ? editor.getPath() : undefined, // as a fallback path
      onlytoplevel: true
    }).then(symbols => {
