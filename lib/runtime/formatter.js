...
function formatEditorTextInRange (editor, range, text) {
  ...
  let indent = atom.config.get('julia-client.juliaOptions.formattettingOptions.indent')
  if (indent === -1) indent = editor.getTabLength()
  let margin = atom.config.get('julia-client.juliaOptions.formattettingOptions.margin')
  if (margin === -1) margin = editor.getPreferredLineLength()
  format({
    text,
    indent,
    margin,
    always_for_in: atom.config.get('julia-client.juliaOptions.formattettingOptions.always_for_in'),
    whitespace_typedefs: atom.config.get('julia-client.juliaOptions.formattettingOptions.whitespace_typedefs'),
    whitespace_ops_in_indices: atom.config.get('julia-client.juliaOptions.formattettingOptions.whitespace_ops_in_indices'),
    remove_extra_newlines: atom.config.get('julia-client.juliaOptions.formattettingOptions.remove_extra_newlines')
