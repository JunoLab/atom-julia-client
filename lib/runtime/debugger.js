...
function toggleJuliaBP (file, line) {
  client.import('toggleBP')(file, line+1).then(response => {
    if (response) {
      breakpoints.setBreakpoints(response)
    }
  })
}
...
export function togglebp(ed = atom.workspace.getActiveTextEditor()) {
  ...
  ed.getCursors().map((cursor) =>
    toggleJuliaBP(ed.getPath(), cursor.getBufferPosition().row))
}
...
export function consumeInk(i) {
  ...
  breakpoints = new ink.breakpoints('source.julia', toggleJuliaBP)
