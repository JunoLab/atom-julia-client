...
var { changemodule, validatepath, fullpath } = client.import({msg: ['changemodule'], rpc: ['validatepath', 'fullpath']})
...
export function activate (ink) {
  ...
  client.onBoot((proc) => {
    ...
    let linkHandler = (event, uri) => {
      [uri, line] = uri.split(':')
      fullpath(uri).then((path) => ink.Opener.open(path, parseInt(line) - 1))
    }
