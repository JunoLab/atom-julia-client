...
export function activate (ink) {
  ...
  terminal.setTitle('REPL')
  terminal.class = 'julia-terminal'

  terminal.terminal.setOption('scrollback', atom.config.get('julia-client.consoleOptions.maximumConsoleSize'))
  terminal.terminal.setOption('cursorStyle', atom.config.get('julia-client.consoleOptions.cursorStyle'))
  ...
  atom.workspace.getPaneItems().forEach((item) => {
    if (item.id && item.name === 'InkTerminal' && item.id.match(/terminal\-julia\-\d+/)) {
      if (!item.ty) {
        ...
        item.terminal.setOption('scrollback', atom.config.get('julia-client.consoleOptions.maximumConsoleSize'))
        item.terminal.setOption('cursorStyle', atom.config.get('julia-client.consoleOptions.cursorStyle'))
