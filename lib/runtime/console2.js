...
export function activate (ink) {
  ...
  terminal = ink.InkTerminal.fromId('julia-terminal', {scrollback: atom.config.get('julia-client.maximumConsoleSize')})
  ...
  subs.add(atom.commands.add('atom-workspace', 'julia-client:new-terminal', () => {
    let term = ink.InkTerminal.fromId(`terminal-julia-${Math.floor(Math.random()*10000000)}`, {scrollback: atom.config.get('julia-client.maximumConsoleSize')})
