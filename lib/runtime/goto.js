...
class Goto {
  ...
  getJumpFilePath(editor, bufferPosition) {
    ...
    let match = text.match(includeRegex)

    if (!match) return false

    match = text.match(filePathRegex)
    const filePathBody = match[0].replace(/"/g, '')
    const dirPath = path.dirname(editor.getPath())
    const filePath = path.join(dirPath, filePathBody)
    const columnStart = match.index + 1
    const columnEnd = columnStart + filePathBody.length
    if (columnStart <= column && column <= columnEnd) {
      const range = new Range(new Point(row, columnStart), new Point(row, columnEnd))
