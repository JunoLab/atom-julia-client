...
function getRanges (ed, row) {
  const ranges = row ?
    [{
      selection: undefined,
      range: getRange(ed, row)
    }] :
    ed.getSelections().map(sel => {
      return {
        selection: sel,
        range: sel.isEmpty() ?
  ...
  return ranges.filter(({ range }) => {
    return range != null && ed.getTextInBufferRange(range).trim()
  })
}

export function get (ed, row) {
  return getRanges(ed, row).map(({ range, selection }) => {
    return {
      range,
      selection,
      line: range[0][0],
...
export function getLocalContext (editor, row) {
  ...
  return {
    context: block ? block.text : '',
    startRow:  block ? block.line : 1
  }
}
