...
/**
 * Returns a `Promise` that either `resolve`s to a started Julia process
 * or is `reject`ed with the error that occurred.
 * The returned process has the following fields:
 *   - ty: node-pty pesudo tty object
 *   - socket: socket used for communication between Atom and Julia
 *   - onExit: Allows registering a listener that's called when the Julia process exits
 *   - onStdout: Allows registering a listener that's called when something is printed to STDOUT
 *   - kill: Kill the Julia process
 *   - interrupt: Send a SIGINT to the Julia process (or to the PS wrapper on Windows)
 *   - interruptREPL: Send a ^C to the PTY, which allows safely intterupting REPL execution
 */
